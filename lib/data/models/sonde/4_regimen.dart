...
class Regimen {
  ...
  factory Regimen.fromMap(Map<String, dynamic>? map) {
    if (map == null) return initialRegimen();
    return Regimen(
      medicalActions:
  ...
  Regimen clone() {
    return Regimen(
      medicalActions: [for (dynamic x in medicalActions) x.clone()], name: name,
    );
  }
...
// Kiểm tra xem medicalaction cuối ở rangetime trước
  ...
  DateTime lastTime() {
    if (medicalActions.length == 0) return DateTime(1999);
    return medicalActions.last.time;
  }

  DateTime firstTime() {
    if (medicalActions.length == 0) return DateTime(1999);
    return medicalActions.first.time;
  }
